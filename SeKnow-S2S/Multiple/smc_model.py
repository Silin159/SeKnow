...
class SemiBootstrapSMC(BaseModel):
    ...
    def forward_turn(self, u_input, turn_states, is_train, m_input=None, z_input=None, t_input=None,
        ...
        pt_prob, pt_samples = \
            self.decode_t_T(batch_size, u_hiddens, t_input, t_input_1hot, turn_states, sample_type,
                            context_to_response=context_to_response)

        # P(z|pv_z, u) domain-slot-value decoding, as well as the combination and encoding of z
        pz_prob, pz_samples, z_hiddens, turn_states = \
            self.decode_z_T(batch_size, u_hiddens, z_input, z_input_1hot, pt_prob, pt_samples, turn_states,
                            sample_type=sample_type, context_to_response=context_to_response)
        ...
        pz_prob, pz_samples, z_hiddens, turn_states = \
            self.decode_z(batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, z_input, z_input_1hot,
                          pt_prob, pt_samples, turn_states, sample_type=sample_type, decoder_type='pz',
                          context_to_response=context_to_response)
        '''
