...
class BaseModel(nn.Module):
    def __init__(self, cfg, reader, has_qnet):
        ...
        self.pos_encoding = PositionalEncoding(cfg.embed_size, cfg.pos_max_len, cfg.dropout_rate)
        ...
        self.pt_decoder = PriorDecoder_Pz_T(self.embedding, self.pos_encoding, cfg.vocab_size, cfg.hidden_size,
    ...
    def decode_t_T(self, batch_size, u_hiddens, t_input, t_input_1hot, turn_states, sample_type,
        ...
        if context_to_response:
            t_samples.append(t_input)
            t_prob.append(t_input_1hot)
            ...
            prob_out, hidden_out = self.pt_decoder.forward(t_input_shift_emb, u_hiddens, pv_h,
                                                           input_type='embedding', is_train=True)
            t_prob.append(prob_out)
        else:
            prev_t = None
            ...
            for t in range(self.t_length):
                ...
                t_samples.append(tt.view(-1, 1))
                t_prob.append(gen_prob.unsqueeze(1))

        return t_prob, t_samples

    def decode_z_T(self, batch_size, u_hiddens, z_input, z_input_1hot, pt_prob, pt_samples,
    ...
    def decode_z_order(self, batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, z_input, z_input_1hot,
                       pt_prob, pt_samples, turn_states, sample_type, decoder_type, qz_samples=None, qz_hiddens=None,
