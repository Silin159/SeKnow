...
class Model:
    ...
    def train(self, pretrain=False, skip_sup=False, ctr=False):
        self.final_epoch = 0
        # total_step = 0
        # warm_up_step = cfg.warmup_steps

        prev_min_loss, early_stop_count = 1e9, cfg.early_stop_count
        weight_decay_count = cfg.weight_decay_count
        st = time.time()
        # lr = cfg.lr
        # optim = torch.optim.Adam(params=filter(lambda x: x.requires_grad, self.model.parameters()), lr=cfg.lr,
        ...
        for epoch in range(max_epoch):
            ...
            lr = cfg.lr / math.pow(epoch+1, cfg.decay_rate)
            ...
            for iter_num, dial_batch in enumerate(self.reader.batches['train']):
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    optim.zero_grad()
                    # total_step += 1
                    # lr = cfg.lr / max(math.sqrt(total_step), warm_up_step*math.sqrt(warm_up_step)/total_step)
                    # for group in optim.param_groups:
                    #     group['lr'] = lr
            ...
            for dial_batch in self.reader.batches[data]:
                ...
                for turn_num, turn_batch in enumerate(dial_batch):
                    ...
                    losses, turn_states_loss = self.model(u_input=u_input, z_input=z_input, m_input=m_input,
                                                          ...
                                                          a_input=a_input, db_vec=db_vec, filling_vec=filling_vec,
                                                          context_to_response=ctr, amend=cfg.entity_amend)
