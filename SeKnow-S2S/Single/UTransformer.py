...
import numpy as np
import math
from common_layer import EncoderLayer, DecoderLayer, LayerNorm, LayerNorm_No_Grad, _gen_bias_mask, _gen_timing_signal
...
class BabiUTransformer(nn.Module):
    ...
    def __init__(self, num_vocab, embedding_size, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
        ...
        self.transformer_enc = UT_Encoder(embedding_size, hidden_size, num_layers, num_heads, total_key_depth, total_value_depth,
...
class UT_Encoder(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_layers, num_heads, filter_size, max_length=200, input_dropout=0.1,
                 layer_dropout=0.1, attention_dropout=0.1, relu_dropout=0.1, use_mask=False, act=False):
        """
        Parameters:
        ...
        super(UT_Encoder, self).__init__()
    ...
    def forward(self, inputs):
        ...
        if self.act:
            x, (remainders, n_updates) = self.act_fn.forward(x, inputs, self.enc, self.timing_signal, self.position_signal, self.num_layers)
...
class UT_Decoder(nn.Module):
    ...
    """
    def __init__(self, hidden_size, num_layers, num_heads, filter_size, max_length=200, input_dropout=0.1,
                 layer_dropout=0.1, attention_dropout=0.1, relu_dropout=0.1, act=False):
        """
        Parameters:
            hidden_size: Hidden size
            num_layers: Total layers in the Encoder
        ...
        super(UT_Decoder, self).__init__()
        ...
        self.layer_norm_no_grad = LayerNorm_No_Grad(hidden_size)
        self.input_dropout = nn.Dropout(input_dropout)
        if self.act:
            self.act_fn = ACT_basic(hidden_size)
    
    def forward(self, inputs, encoder_output, is_train=True):
        #Add input dropout
        x = self.input_dropout(inputs)
        ...
        if self.act:
            x, (remainders, n_updates) = self.act_fn.forward(x, inputs, self.dec, self.timing_signal, self.position_signal, self.num_layers, encoder_output)
            return x, (remainders, n_updates)
        else:
            for l in range(self.num_layers):
                x += self.timing_signal[:, :inputs.shape[1], :].type_as(inputs.data)
                x += self.position_signal[:, l, :].unsqueeze(1).repeat(1, inputs.shape[1], 1).type_as(inputs.data)
                x, _ = self.dec.forward((x, encoder_output))
            # if is_train:
            #     x = self.layer_norm_no_grad.forward(x)
        return x
