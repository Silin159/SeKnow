...
class EncoderLayer(nn.Module):
    ...
    def forward(self, inputs):
        ...
        y = self.multi_head_attention.forward(x, x, x)
        
        # Dropout and residual
        x = self.dropout(x + y)
        
        # Layer Normalization
        x = self.layer_norm_mha.forward(x)
        
        # Positionwise Feedforward
        y = self.positionwise_feed_forward.forward(x)
        
        # Dropout and residual
        x = self.dropout(x + y)

        # Layer Normalization
        x = self.layer_norm_ffn.forward(x)
        '''

        # Layer Normalization
        x_norm = self.layer_norm_mha.forward(x)

        # Multi-head attention
        y = self.multi_head_attention.forward(x_norm, x_norm, x_norm)

        # Dropout and residual
        x = self.dropout(x + y)

        # Layer Normalization
        x_norm = self.layer_norm_ffn.forward(x)

        # Positionwise Feedforward
        y = self.positionwise_feed_forward.forward(x_norm)
...
class DecoderLayer(nn.Module):
    ...
    """
    def __init__(self, hidden_size, total_key_depth, total_value_depth, filter_size, num_heads,
                 bias_mask, layer_dropout=0.1, attention_dropout=0.1, relu_dropout=0.1):
        """
        Parameters:
            hidden_size: Hidden size
            total_key_depth: Size of last dimension of keys. Must be divisible by num_head
        ...
        self.layer_norm_mha_enc_dec = LayerNorm(hidden_size)
        self.layer_norm_ffn = LayerNorm(hidden_size)
    ...
    def forward(self, inputs):
        ...
        # Layer Normalization before decoder self attention
        x_norm = self.layer_norm_mha_dec.forward(x)

        # Masked Multi-head attention
        y = self.multi_head_attention_dec.forward(x_norm, x_norm, x_norm)

        # Dropout and residual after self-attention
        x = self.dropout(x + y)

        # Layer Normalization before encoder-decoder attention
        x_norm = self.layer_norm_mha_enc_dec.forward(x)

        # Multi-head encoder-decoder attention
        y = self.multi_head_attention_enc_dec.forward(x_norm, encoder_outputs, encoder_outputs)
...
class LayerNorm_No_Grad(nn.Module):
    # Borrowed from jekbradbury
    # https://github.com/pytorch/pytorch/issues/1959
    def __init__(self, features, eps=1e-6):
        super(LayerNorm_No_Grad, self).__init__()
        self.eps = eps

    def forward(self, x):
        mean = x.data.mean(-1, keepdim=True)
        std = x.data.std(-1, keepdim=True)
