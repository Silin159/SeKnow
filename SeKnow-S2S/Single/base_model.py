...
class BaseModel(nn.Module):
    def __init__(self, cfg, reader, has_qnet):
        ...
        self.embedding = nn.Embedding(cfg.vocab_size, cfg.embed_size, padding_idx=0)
        self.pos_encoding = PositionalEncoding(cfg.embed_size, cfg.hidden_size, cfg.pos_max_len, cfg.dropout_rate)

        '''Transformers Version'''

        self.u_encoder = Encoder_T(self.embedding, self.pos_encoding, cfg.hidden_size, cfg.attn_head, cfg.feed_size,
                                   cfg.layer_num, cfg.dropout_rate)
        self.z_encoder = Encoder_T(self.embedding, self.pos_encoding, cfg.hidden_size, cfg.attn_head, cfg.feed_size,
                                   cfg.layer_num, cfg.dropout_rate)
        self.k_encoder = Encoder_T(self.embedding, self.pos_encoding, cfg.hidden_size, cfg.attn_head, cfg.feed_size,
                                   cfg.layer_num, cfg.dropout_rate)
        ...
        '''Transformers Version'''
        self.pz_decoder = PriorDecoder_Pz_T(self.embedding, self.pos_encoding, cfg.vocab_size, cfg.hidden_size,
                                            cfg.attn_head, cfg.feed_size, cfg.layer_num, cfg.dropout_rate)

        '''GRU Version'''
        # self.pt_decoder = TopicDecoder_Pt(cfg.embed_size, cfg.hidden_size, cfg.vocab_size, cfg.dropout_rate)
        # self.feed_trans_to_gru = nn.Linear(cfg.hidden_size, cfg.hidden_size_gru)
        # self.pz_decoder = PriorDecoder_Pz(cfg.embed_size, cfg.hidden_size_gru, cfg.vocab_size,
        #                                   cfg.dropout_rate_gru, enable_selc_read=False)  # prior
        # self.decode_z = self.decode_z_order  # if not cfg.parallel_z else self.decode_z_parallel

        '''Transformers Version'''
        self.m_decoder = ResponseDecoder_Pm_T(self.embedding, self.pos_encoding, cfg.vocab_size, cfg.hidden_size,
                                              cfg.db_vec_size, cfg.attn_head, cfg.feed_size,
                                              cfg.layer_num, cfg.dropout_rate)
    ...
    def decode_t(self, batch_size, u_input, u_hiddens, u_input_1hot, u_last_hidden, t_input, t_input_1hot,
        ...
        return t_prob, t_samples, log_pt
    '''
    ...
    def encode_k(self, kb_matches, k_ground, sample_type, context_to_response=False):
        ...
        k_hiddens = self.k_encoder.forward(k_input, input_type='index')

        return k_hiddens, k_input

    def decode_z_T(self, batch_size, u_hiddens, z_input, z_input_1hot, t_input, t_input_1hot,
                   turn_states, sample_type, context_to_response=False):
        pv_z_t_pr = turn_states.get('pv_pr', None)
        pv_z_t_h = turn_states.get('pv_h', None)
        pv_z_t_id = turn_states.get('pv_id', None)
        z_t_prob, z_t_samples = [], []
        for si, sn in enumerate(self.reader.otlg.informable_slots):
            z_eos_idx = self.vocab.encode(self.z_eos_map[sn])
            start_z = self.get_first_z_input(sn, batch_size, self.multi_domain)  # (B,1,H)
            if pv_z_t_pr is not None:
                b, e = si * self.z_length, (si+1) * self.z_length
                pv_pr, pv_h, pv_idx = pv_z_t_pr[:, b:e], pv_z_t_h[:, b:e], pv_z_t_id[:, b:e]
            else:
                pv_pr, pv_h, pv_idx = None, None, None

            if context_to_response:
                z_t_samples.append(z_input[sn])
                z_t_prob.append(z_input_1hot[sn])
            elif sample_type == 'supervised':
                z_t_samples.append(z_input[sn])
                z_input_emb = self.embedding(z_input[sn])  # (B,T,H)
                if z_input_emb.size(1) > 1:
                    z_input_shift_emb = torch.cat([start_z, z_input_emb[:, :-1, :]], dim=1)
                else:
                    z_input_shift_emb = start_z
                prob_out, hidden_out = self.pz_decoder.forward(z_input_shift_emb, u_hiddens, pv_h,
                                                               input_type='embedding', is_train=True)
                z_t_prob.append(prob_out)
            else:
                prev_z = None
                z_input_shift_emb = start_z
                for t in range(self.z_length):
                    prob_out, hidden_out = self.pz_decoder.forward(z_input_shift_emb, u_hiddens, pv_h,
                                                                   input_type='embedding', is_train=False)
                    gen_prob = prob_out[:, -1, :].squeeze(1)

                    if sample_type == 'top1':
                        zt = torch.topk(gen_prob, 1)[1]
                    elif sample_type == 'topk':
                        topk_probs, topk_words = torch.topk(gen_prob, cfg.topk_num)
                    ...
                    emb_zt = self.embedding(zt.view(-1, 1))  # (B,1,H)
                    z_input_shift_emb = torch.cat([z_input_shift_emb, emb_zt], dim=1)

                    zt, prev_z = self.mask_samples(zt, prev_z, batch_size, z_eos_idx)

                    z_t_samples.append(zt.view(-1, 1))
                    z_t_prob.append(gen_prob.unsqueeze(1))

        # decode topic
        t_sos_idx = self.vocab.encode('<go_t>')
        t_eos_idx = self.vocab.encode(self.eos_t_token)
        start_t_idx = cuda_(torch.ones(batch_size, 1) * t_sos_idx).long()
        start_t = self.embedding(start_t_idx)  # (B,1,H)

        if pv_z_t_pr is not None:
            pv_pr, pv_h, pv_idx = pv_z_t_pr[:, -self.t_length:], pv_z_t_h[:, -self.t_length:], \
                                  pv_z_t_id[:, -self.t_length:]
        else:
            pv_pr, pv_h, pv_idx = None, None, None

        if context_to_response:
            z_t_samples.append(t_input)
            z_t_prob.append(t_input_1hot)
        elif sample_type == 'supervised':
            z_t_samples.append(t_input)
            t_input_emb = self.embedding(t_input)  # (B,T,H)
            if t_input_emb.size(1) > 1:
                t_input_shift_emb = torch.cat([start_t, t_input_emb[:, :-1, :]], dim=1)
            else:
                t_input_shift_emb = start_t
            prob_out, hidden_out = self.pz_decoder.forward(t_input_shift_emb, u_hiddens, pv_h,
                                                           input_type='embedding', is_train=True)
            z_t_prob.append(prob_out)
        else:
            prev_t = None
            ...
            for t in range(self.t_length):
                ...
                if sample_type == 'top1':
                    tt = torch.topk(gen_prob, 1)[1]
                elif sample_type == 'topk':
                    topk_probs, topk_words = torch.topk(gen_prob, cfg.topk_num)
                    widx = torch.multinomial(topk_probs, 1, replacement=True)
                    tt = torch.gather(topk_words, 1, widx)  # [B]
                else:
                    raise Exception("Unknown Sampling Type!")

                emb_tt = self.embedding(tt.view(-1, 1))  # (B,1,H)
                t_input_shift_emb = torch.cat([t_input_shift_emb, emb_tt], dim=1)

                tt, prev_t = self.mask_samples(tt, prev_t, batch_size, t_eos_idx)

                z_t_samples.append(tt.view(-1, 1))
                z_t_prob.append(gen_prob.unsqueeze(1))

        z_t_prob = torch.cat(z_t_prob, dim=1)  # [B,Tz*K+Tt,V], K=len(self.reader.otlg.informable_slots)
        z_t_samples = torch.cat(z_t_samples, dim=1)  # [B,Tz*K+Tt]

        z_t_hiddens = self.z_encoder.forward(z_t_samples, input_type='index')

        retain = self.prev_z_continuous
        turn_states['pv_h'] = z_t_hiddens if retain else z_t_hiddens.detach()
        turn_states['pv_pr'] = z_t_prob if retain else z_t_prob.detach()
        turn_states['pv_id'] = z_t_samples if retain else z_t_samples.detach()

        return z_t_prob, z_t_samples, z_t_hiddens, turn_states

    def decode_m_T(self, batch_size, u_hiddens, z_hiddens, k_hiddens, db_vec, m_input, is_train):
        start_m_idx = cuda_(torch.ones(batch_size, 1).long())  # <go_r> token, id == 1
        start_m = self.embedding(start_m_idx)  # (B,1,H)
        m_idx, pm_prob = [], []
        m_eos_idx = self.vocab.encode(self.eos_m_token)

        if is_train:
            m_idx.append(m_input)
            m_input_emb = self.embedding(m_input)  # (B,T,H)
            if m_input_emb.size(1) > 1:
                m_input_shift_emb = torch.cat([start_m, m_input_emb[:, :-1, :]], dim=1)
            else:
                m_input_shift_emb = start_m
            prob_out, hidden_out = self.m_decoder.forward(m_input_shift_emb, u_hiddens, z_hiddens, k_hiddens,
                                                          db_vec, input_type='embedding', is_train=True)
            pm_prob.append(prob_out)
        else:
            prev_m = None
            m_input_shift_emb = start_m
            for t in range(self.max_len):
                prob_out, hidden_out = self.m_decoder.forward(m_input_shift_emb, u_hiddens, z_hiddens, k_hiddens,
                                                              db_vec, input_type='embedding', is_train=False)
                gen_prob = prob_out[:, -1, :].squeeze(1)
                mt = torch.topk(gen_prob, 1)[1]

                emb_mt = self.embedding(mt.view(-1, 1))  # (B,1,H)
                m_input_shift_emb = torch.cat([m_input_shift_emb, emb_mt], dim=1)

                mt, prev_m = self.mask_samples(mt, prev_m, batch_size, m_eos_idx)

                m_idx.append(mt.data.view(-1, 1))
                pm_prob.append(gen_prob.unsqueeze(1))

        pm_prob = torch.cat(pm_prob, dim=1)  # [B,T,V]
        m_idx = torch.cat(m_idx, dim=1)
        m_idx = [list(_) for _ in list(m_idx)]
        return pm_prob, m_idx

    def decode_z_order(self, batch_size, u_input, u_hiddens, u_input_1hot, z_input, z_input_1hot,
        ...
        u_hiddens_to_gru = self.feed_trans_to_gru(u_hiddens)
        last_hidden = u_hiddens_to_gru[:, -1, :].squeeze(1).unsqueeze(0).contiguous()
        ...
        for si, sn in enumerate(self.reader.otlg.informable_slots):
            ...
            if pv_h is not None:
                pv_h_to_gru = self.feed_trans_to_gru(pv_h)
            else:
                pv_h_to_gru = None
            ...
            for t in range(self.z_length):
                if decoder_type == 'pz':
                    prob, last_hidden, gru_out, selc_read_u, selc_read_pv_z = \
                        self.pz_decoder.forward(u_input, u_input_1hot, u_hiddens_to_gru,
                                                pv_z_prob=pv_pr, pv_z_hidden=pv_h_to_gru, pv_z_idx=pv_idx,
                    ...
                    prob, last_hidden, gru_out, selc_read_u, selc_read_m, selc_read_pv_z, gmb_samp = \
                        self.qz_decoder.forward(u_input, u_input_1hot, u_hiddens_to_gru, m_input, m_input_1hot, m_hiddens,
                                                pv_z_prob=pv_pr, pv_z_hidden=pv_h_to_gru, pv_z_idx=pv_idx,
                ...
                zt, prev_zt = self.mask_samples(zt, prev_zt, batch_size, z_eos_idx)
        ...
        for t in range(self.t_length):
            ...
            if sample_type == 'supervised' or context_to_response is True:
                ...
                tt = torch.topk(prob, 1)[1]
            emb_tt = self.embedding(tt.view(-1, 1))
            tt, prev_tt = self.mask_samples(tt, prev_tt, batch_size, t_eos_idx)
        ...
        if sample_type == 'posterior':
            ...
            z_hiddens = self.z_encoder.forward(z_samples, input_type='index')
        else:
            z_gumbel = torch.stack(gmb_samples, dim=1)   # [B,Tz,V]
            z_gumbel = torch.matmul(z_gumbel, self.embedding.weight)     # [B,Tz,E]
            z_hiddens = self.z_encoder.forward(z_gumbel, input_type='embedding')
        ...
        return z_prob, z_samples, z_hiddens, turn_states
    ...
    def mask_samples(self, curr_z, prev_z, batch_size, eos_idx):
