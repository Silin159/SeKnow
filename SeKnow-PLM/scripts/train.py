...
class Trainer:
    ...
    def _initialize_dataloaders(self) -> torch.utils.data.Dataset:
        ...
        if self.args.backtranslations != 'none':
            ...
            # randomly replace each user utterance with its backtranslated paraphrases
        ...
        # 0: item[0], 1: negative(item[0]), 2: item[1], 3: negative(item[1]), ...
        train_dataset = data.NegativeSamplingDatasetWrapper(dataset, train_transform)
        # Original sampler samples 0, wrapped sampler samples 0 and 1, corresponding positive item and its negative
    ...
    def _load_model(self):
        ...
        self.tokenizer.add_special_tokens({'additional_special_tokens': ['<|eod|>']})  # add token of document end
    ...
    def _initialize_distributed_data_parallel(self, model):
        if self.args.local_rank != -1:
            device_idx = (self.args.local_rank + self.args.device_shift) % len(rank_to_device)
            model = DistributedDataParallel(
                model,
                device_ids=[rank_to_device[device_idx]],
                output_device=rank_to_device[device_idx],
...
def parse_args():
    ...
    parser.add_argument('--device-shift', default=0, type=int)
